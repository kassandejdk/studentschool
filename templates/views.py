...
def professor(request):
    ...
    course = Course.objects.filter(niveau=request.user.niveau)
    queryset_course = Course.objects.filter(Q(user__username=request.user) and Q(niveau=request.user.niveau)).prefetch_related('questions')
    queryset_courses = Course.objects.filter(Q(user__username=request.user) and Q(niveau=request.user.niveau)).prefetch_related('choices')
    ...
    context = {
        ...
        "queryset_course": queryset_courses,
...
def student_course(request, course_name, slug=None):
    ...
    if user in course.students.all() or user.is_professor or user.is_site_admin or course.for_everybody:
        ...
        print('resultat list : ',chapter.id,videos,Video.objects.all(),result_list)
